...
class AIHubService:
    ...
    def __init__(self):
        ...
        # Priority: APP_AI_KEY/APP_AI_BASE_URL (proxy) > OPENAI_API_KEY (direct)
        try:
            ai_key = settings.app_ai_key
        except AttributeError:
            ai_key = None
        try:
            ai_base_url = settings.app_ai_base_url
        except AttributeError:
            ai_base_url = None

        # Fallback to OPENAI_API_KEY for direct OpenAI connection
        if not ai_key:
            ai_key = os.environ.get("OPENAI_API_KEY")

        if ai_key:
            client_kwargs: dict = {"api_key": ai_key}
            if ai_base_url:
                client_kwargs["base_url"] = ai_base_url.rstrip("/")
